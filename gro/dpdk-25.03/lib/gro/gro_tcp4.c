		...
		uint64_t start_time)
{
	...
	if (unlikely(INVALID_TCP_HDRLEN(pkt->l4_len)))
		{line = 138; rte_gro_trace_tcp4_reassemble_error_line(line); return -1;}
	...
	if (tcp_hdr->tcp_flags & ~VALID_GRO_TCP_FLAGS)
		{line = 147; rte_gro_trace_tcp4_reassemble_error_line(line); return -1;}
	...
	if (tcp_dl <= 0)
		{line = 160; rte_gro_trace_tcp4_reassemble_error_line(line); return -1;}
	...
	if (find == 1) {
		...
		if (tcp_hdr->tcp_flags & (RTE_TCP_ACK_FLAG | RTE_TCP_PSH_FLAG | RTE_TCP_FIN_FLAG)) {
			...
			line = 207; rte_gro_trace_tcp4_reassemble_error_line(line); return -1;
		}
	}
	/*
	...
	if (tcp_hdr->tcp_flags == RTE_TCP_ACK_FLAG) {
		...
		if (item_idx == INVALID_ARRAY_INDEX)
			{line = 221; rte_gro_trace_tcp4_reassemble_error_line(line); return -1;}
		if (insert_new_flow(tbl, &key, item_idx) ==
			...
			*/
			...
			line = 229; rte_gro_trace_tcp4_reassemble_error_line(line); return -1;
		}
		return 0;
	}

	line = 234; rte_gro_trace_tcp4_reassemble_error_line(line); return -1;
}
